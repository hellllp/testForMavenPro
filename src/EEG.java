...
class NewWindow extends JFrame {
    ...
    private long TEST_PART_A = 20 * 1000, TEST_PART_B = 20 * 1000, TEST_PART_CLOCK = 10;

    private JFrame clock_win = new JFrame("中间休息一分钟");
    ...
    private void show_time(long t_) {
        ...
        while (t_ >= 0) {
            ...
            clock_text.setText("<html>请保持安静，身体尽量放松<br>避免任何动作（包括眼，手，身体其他部位）<br>"
                +"倒计时结束该对话框将自动消失<br><br>"+hour+" 时 "+minute+" 分 "+seconds+" 秒</html>");
        ...
        try {
            ...
            if (mode == TEST_MODE) {
                ...
                if (second && temp_time - start_time_2 > TEST_PART_B) {
					...
					JFrame end_win = new JFrame();
					end_win.setSize(window_w, window_h);
					JPanel end_jp = new JPanel();
					end_jp.setLocation(window_w / 2, window_h / 2);
					JLabel end_text = new JLabel("<html><br><br><br>实验结束，感谢参与！</html>");
					end_text.setFont(new Font("Serif", Font.BOLD, 80));
					end_jp.add(end_text);
					end_win.add(end_jp);
					end_win.setVisible(true);
                    second = false;
					Thread.sleep(10000);
                    System.exit(0);
                } 
